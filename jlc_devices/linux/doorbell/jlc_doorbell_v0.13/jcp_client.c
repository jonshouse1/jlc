...
int jcp_bcast=0;
int jcp_waiting_ack=-1;								// -1 not wating for any ack
...
void ICACHE_FLASH_ATTR jcp_init_b(char *uidb, uint16_t sid, int jcp_udp_iport, int jlp_udp_iport, int tick_interval_ms, char* device_model, char* device_fw_ver)
{
	...
	session_id=sid;								// Should be a non zero randon number
...
// The position of the 1/0 digit commisponds to JCP_TOPIC definitions, leftmost is JCP_TOPIC_STATUSLINE
...
// Normally the UID is the base UID (mac) with the last digit the idx, allow user to override that with a arbitary UID
...
// Take a pre-built message payload (message exluding header) and send it. Every outbound message should be via this.
// send_ack tells the server to send a reply to the message, requires_ack asks jcp_client to possibly re-try
// sending and wait for positve ack to confirm the server received the message
...
// Re-send the last massges unchanged
...
// The ACK aknowledges reciept and having acted upton (even if partially) a message.
void ICACHE_FLASH_ATTR jcp_send_ack(char *ipaddr, int message_id, int port)
{
	...
	jcph.send_ack=FALSE;                                                            // We dont want a loop trying to ACK the ACK
