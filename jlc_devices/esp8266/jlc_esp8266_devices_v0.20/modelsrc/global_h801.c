...
#pragma message ( "Building for H801" )
...
{
        os_printf("jcp_dev_state_cb()\tidx=%d\n");
	if (idx==-100)										// jcp_client.c did device registration
		return;
...
static ICACHE_FLASH_ATTR void process_jlp_packet(char *ipaddr, unsigned char *data, int len)
{
	...
	os_printf("Good packet from %s, NOV:%d ",ipaddr,p->nov);
// p->voffset should always be zero , check maybe ?
