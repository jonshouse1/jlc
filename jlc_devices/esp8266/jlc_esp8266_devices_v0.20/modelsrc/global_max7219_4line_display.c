// global_max7219_4line_display.c
#pragma message ( "Building for MAX7219_4LINE_DISPLAY" )
...
#define DISP_LINES	4							// 4 lines
#define DISP_WIDTH	32							// of 32 pixels wide
#define DISP_HEIGHT	8							// by 8 high
__attribute__((aligned(32))) unsigned char disp[DISP_LINES][DISP_HEIGHT][DISP_WIDTH];
...
void ICACHE_FLASH_ATTR select_module(int m)
{
        //void max_setgpio(int pin_data, int pin_clock, int pin_chipsel)

        os_printf("select_module %d\n",m);
        switch (m)
        {
                case 0:
                        max_setgpio(4, 0, 5);
...
// Select a single 32x8 display and update it
void update_display(int l)
{
	int nc=0;
	int lines=0;
	int pixels;
	...
	for (nc=0;nc<4;nc++)											// num of controllers
	{
		for (lines=0;lines<DISP_HEIGHT;lines++)								// for each line of leds
		{
			...
			for (bytes=0;bytes<4;bytes++)								// 4 bytes per line of pixels
			{
				...
				{
					p=8-(pixels+1); 
					b[bytes] |= disp[l][lines][p+(bytes*8)]<<pixels;			// turn bytes into bits
				}	
			}
...
// Line number, string, rightposition
void display_text(int l,char *st, int moveright)
{
	...
	render_text_to_buffer_small(moveright, st,strlen(st), (char*)&disp[0][0],sizeof(disp), 32, 8, TRUE, 1);
	...
	if (alignbot==TRUE)								// Move display down one line and clear the bot line
	{
		for (r=7;r>=0;r--)
		{
			for (i=0;i<31;i++)
			{
				disp[l][r][i]=disp[0][r-1][i];
			}
		}
		for (i=0;i<32;i++)
			disp[l][0][i]=0;						// clear top line
	}
	update_display(l);
}
...
void slow_timer_cb()
{
	...
	//{
		//popupmsg--;
		//display_text(0,popupmsg_string,0);
		//return;
	//}
	...
	//{
		...
		//bzero(&st,sizeof(st));
	//}
	//display_text(0,st,2);										// normal clock display, move it right a bit
}
...
void jcp_dev_state_cb(int idx, struct dev_state* ds)
{
	...
	// value1	value2 
	//  0		0		Reset the display
	//  1		0 to 15		Set brightness
	//  2		line num	Display message 
	...
	if (idx==0)
	{
		switch (ds->value1)
		{
			...
			case 2:
				...
				if (ds->value1>=2 && ds->value2<=4)
				{
					display_text(ds->value2-1,ds->valuebytes,0);
				}
				else	os_printf("ds->value2 not in range 1 to 4\n");
			break;
