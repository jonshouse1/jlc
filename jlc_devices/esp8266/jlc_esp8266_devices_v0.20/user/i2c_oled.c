...
// TRUE = on
void ICACHE_FLASH_ATTR set_ledsaver(int f)
{
	ledsaver=f;
}


void ICACHE_FLASH_ATTR toggle_invert()
{
	inverted=~inverted;
}


//void ICACHE_FLASH_ATTR oled_flip(int tf)
//{
	//if (tf==0)
  		//OLED_writeCmd(0xc0); 
	//if (tf==1)
...
OLED_Init(void)
{
  ...
  OLED_writeCmd(0x3f); // 1/64 duty "MUX Ratio"
...
{
    unsigned char m,n;
system_soft_wdt_feed();             // JA, try to stop watchdog resets
    for(m=0;m<8;m++)
    {
        ...
        case 1:  // Small font, 6x8
        {
            ...
            while (ch[index] != '\0')
            {
                ...
                for (charCol = 0; charCol < 6; charCol++)
                {
                    if ( inverted==0 )
  			     OLED_writeCmd(0xa6); 				// set normal display 
  		    else     OLED_writeCmd(0xa7); 				//  invert
