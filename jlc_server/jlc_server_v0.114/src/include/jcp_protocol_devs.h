...
#define JLP_SERVER_PORT				1181				// if avialable default port for JLP (light) protocol
...
#define MAX_VALUEBYTESLEN			512				// Upto N bytes in vlauebytes of dev_state
#define MAX_TOPICS				16				// One flag per topic
#define MAX_TDATA_LEN				16384				// Upto N btyes of data per topic message
//#define MAX_COLOUR_PRESETS			16
...
#define NUM_DEVN_TYPES				15				// Number of DEVN_ types listed below
...
#define DEVN_SWITCHPBM				502				// Push button momentry, device sends 1 when first pushed in, 0 after release
#define DEVN_DOORBELL				510				// momentry action push button
...
#define DEVN_CLOCKCAL				800				// Clock (and or) calander
...
// Colour KEY related
#define MASK_KEY "M=Mono R=Red  G=Green  B=Blue  Y=Yellow  W=White  O=Orange  P=Purple  I=pInk  U=UV  X=Unused"
#define VALID_MASK				"MRGBYWOPIUX"
#define MASK_M					'M'
#define MASK_R					'R'
#define MASK_G					'G'
#define MASK_B					'B'
...
struct __attribute__ ((__packed__)) jcp_header
{
	...
	uint16_t		msg_type;
	uint16_t		session_id;					// uniqu number for session, used by client and server
	uint16_t		message_id;					// the same message may repeat, message_id identifies a uniqu msg
...
struct __attribute__ ((__packed__)) jcp_dev
{
	...
	uint16_t		jlp_udp_port;					// device listens for lighing data on port, 0=unused
...
// Sent from the server to the client, normaly containing details of some sort of event
...
struct __attribute__ ((__packed__)) jlc_devf
{
	uint16_t		dev_type;
	unsigned char		uid[UID_LEN];
...
struct __attribute__ ((__packed__)) jlc_devn
{
...
struct __attribute__ ((__packed__)) jlc_colourpreset
{
        uint16_t        active;
        uint16_t        R;
        uint16_t        G;
